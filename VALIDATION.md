...
### T8 — launch hygiene (2026-09-03)
- supabase/phase7.sql (RUN IT): reports table (players can flag someone
  from their profile card; insert-only, 5/day + one-per-person-per-day
  enforced in the policy; NO client can read them — dashboard only) and
  site_stats + visit_ping() (a privacy-respecting counter: one integer
  per day, no identifiers; read it in the dashboard).
- Profile cards: a quiet "⚑ report" affordance on real non-self cards →
  confirm → insert → "reported — thank you for looking out ♪". Blocks
  remain the immediate self-defense; reports are the review trail.
- Presence join throttle: room changes churn at most one channel join
  per 1.5s (trailing — the last destination always wins), so
  room-flapping or a scripted hammer can't spam the realtime server.
- OG/share: public/og.png (1200×630, generated by the game's own
  wordmark via scripts/gen-og.mjs + playwright from iso-prototype),
  og:image + twitter:card meta point at the absolute URL. Links now
  unfurl with the brand card.
- Visit ping fires once per boot after sign-in; pre-migration it's a
